# -*- coding: utf-8 -*-
'''
@author   : Ahmet Burak Özdemir < burakozdemir@gmail.com >
...
class MessageBox( QtGui.QMainWindow ) :
    ...
    def Show(self, data = {}, S = 1.0, title = _t("Dişlinin güvenlik hesapları") ) :
        ...
        s_icon = QtGui.QIcon()
        s_icon.addPixmap(QtGui.QPixmap( os.path.dirname( sys.argv[0] ) + "\\icons\\tick.png" ), QtGui.QIcon.Normal, QtGui.QIcon.Off)
...
class TableWindow( QtGui.QMainWindow ) :
    ...
    def set( self, table, index ) :
        self.index = index
        if ( table == '1' ) :
            image = os.path.dirname( sys.argv[0] ) + "\\tables\\Yf.png"
            self.showXY = True
        elif ( table == '2' ) :
            image = os.path.dirname( sys.argv[0] ) + "\\tables\\Kc.png"
            self.showXY = False
...
class MainWindow( QtGui.QMainWindow ) :
    ...
    def Kaydet( self ) :
        ...
        if ( 'SHde' in self.O['disli']['1']['e'].keys() ) :
            ...
            icon = QtGui.QIcon( )
            icon.addPixmap(QtGui.QPixmap( os.path.dirname( sys.argv[0] ) + "\\icons\\tick.png" ), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def ButtonHesapla( self ) :
        ...
        if not self.__Kontrol1() :
            ...
            if self.O['it'] < 6.0 :
            ...
            elif self.O['it'] > 6.0 and self.O['it'] < 45.0 :
                ...
                self.label_17.setText(_t("En fazla 2 kademe için hesap yapılabilir, devir sayılarını daha yakın seçiniz !"))


        else :
            self.label_17.setText(_t("Girilmesi gereken tüm veriler girilmedi !"))

        if ( self.O['it'] < 6.0 and not self.__Kontrol3() ) or ( self.O['it'] > 6.0 and self.O['it'] < 45.0 and not self.__Kontrol2() ) :
            ...
            if self.O['it'] < 6.0 : 
                ...
                self.label_17.setText(_t("En fazla 2 kademe için hesaplamalar yapılabilir !"))
        else :
            self.label_17.setText(_t("Girilmesi gereken tüm veriler girilmedi !"))
